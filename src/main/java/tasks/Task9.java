...
//    }
  ...
  void listVsSet() {
    ...
    // в хэш мап используется побитовое И с числом емкости таблицы, а
    // т.к. у нас упакованы числа от 1 до 10000, а размер выделенный под таблицы будет равен 2^14,
    // поэтому не произойдет коллизии при выборе ячейки для хранения числа в таблице, и когда во время преобразования в строку
    // будет итерация по ячейкам в порядке возрастания, то в каждой ячейке будет по 1 числу,
    // если бы чисел было больше чем количество ячеек, то произошла бы коллизия,
    // и числа бы были перемешаны, а не в точном порядке возрастания
